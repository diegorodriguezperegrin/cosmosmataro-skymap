...
enum class SolarSystemBody(
    ...
    Pluto(TimeConstants.MILLISECONDS_PER_HOUR),
    Neptune(TimeConstants.MILLISECONDS_PER_HOUR),
    Uranus(TimeConstants.MILLISECONDS_PER_HOUR),
    Saturn(TimeConstants.MILLISECONDS_PER_HOUR),
    Jupiter(TimeConstants.MILLISECONDS_PER_HOUR),
    Mars(TimeConstants.MILLISECONDS_PER_HOUR),
    Sun(TimeConstants.MILLISECONDS_PER_HOUR),
    Mercury(TimeConstants.MILLISECONDS_PER_HOUR),
    Venus(TimeConstants.MILLISECONDS_PER_HOUR),
    Moon(TimeConstants.MILLISECONDS_PER_MINUTE),
    Earth(TimeConstants.MILLISECONDS_PER_HOUR);
    ...
    // 3000 BC to 3000 AD.
    fun getOrbitalElements(date: Long): OrbitalElements {
    ...
    // New method to get coordinates directly (supporting VSOP87 & ELP2000)
    fun getHeliocentricCoordinates(date: Long): Vector3 {
        if (this == Moon) {
            // Moon is Geocentric in ELP2000.
            // Moon_Helio = Earth_Helio + Moon_Geo
            val earthHelio = Earth.getHeliocentricCoordinates(date)
            val moonGeo = ELP2000LunarTheory.getGeocentricCoordinates(date)
            return earthHelio + moonGeo
        }
        
        // Hybrid Approach: Try VSOP87, fallback to Kepler
        return try {
            VSOP87PlanetaryTheory.getHeliocentricCoordinates(this, date)
        } catch (e: NotImplementedError) {
            com.google.android.stardroid.math.heliocentricCoordinatesFromOrbitalElements(this.getOrbitalElements(date))
        }
    }
